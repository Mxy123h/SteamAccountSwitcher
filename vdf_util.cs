        ...
        private void SetProperty(PropertyInfo destinationProperty, IDictionary<string, object> sourceValue, object objectInstance)
        {
            ...
            if (!string.IsNullOrEmpty(key))
            {
                ...
                if (propertyType == typeof(bool))
                {
                    ...
                    destinationProperty.SetValue(objectInstance, (string)val == "1", null);
                }
                else if (propertyType == typeof(DateTime))
                {
                    // if the type is boolean, then this source type would have been read as a string (or should have been)
                    destinationProperty.SetValue(objectInstance, DateTime.Parse((string)val), null);
                }
                else if (val.GetType() == typeof(ExpandoObject))
                {
                    ...
                    destinationProperty.SetValue(objectInstance, Map(propertyType, val as ExpandoObject, colType), null);
                }
                else
                {
                    destinationProperty.SetValue(objectInstance, (string)val, null);
                }
            }
        }
